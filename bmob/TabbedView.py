...
class TabbedView(ui.View):
    def __init__(self, tablist=[], frame=(0, 0) + ui.get_screen_size()):
        ...
        # setup button bar
        self.tabbuttons = ui.SegmentedControl(
            frame=(0, 0, self.width, self.buttonheight))
    ...
    def focus_tab_by_name(self, tabname):
        self.tabbuttons.selected_index = self.tabbuttons.segments.index(
            tabname)
        self.tab_action(self.tabbuttons)
    ...
    def addtab(self, tab):
        ...
        tab.frame = (0, self.buttonheight, self.width,
    ...
    def removetab(self, tabname):
        self.tabbuttons.segments = [
            x for x in self.tabbuttons.segments if x != tabname]
        self.remove_subview(tabname)
        # if tab was top tab, think about updating selected tab to whatever is
        # on top
